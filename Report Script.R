...
crosswalk <- read_csv('crosswalk_full.csv') 
...
comb_limited <- comb %>% 
  filter(site=="Cabrillo I" | site=="Cabrillo III") %>% 
  filter(year == 2002 | year==2004 & site=="Cabrillo I" | year==2020 & site=="Cabrillo I" | year==2012 & site=="Cabrillo III" | year==2017 & site=="Cabrillo III")

# Create taxa table and figure --------------------------------------------

#Clean and widen crosswalk df
crosswalk <- crosswalk %>% 
  rename("LTM 1990" = "name_1990", "2000" = "name_2000", "LTM current" = "species_code", "CBS" = "ucsc") #Rename columns
wide <- crosswalk %>% 
  pivot_longer(
    cols = c(10:17), 
    names_to = "program", 
...
#Filter df for programs of interest and relevel taxa types
totals <- wide %>% 
  ...
  mutate(type = fct_relevel(type, c("mobile invertebrates", "sessile invertebrates", "algae"))) 

#Create figure
taxa_figure <- ggplot(totals, mapping=aes(x= reorder(program, -n), y = n, fill=type)) + 
  geom_col(width=1) + 
  clean+labs(x = "Program", y = "Number of Taxa")+ 
  scale_fill_manual(values=c("#212E52","#FCC893","#D8511D")) 
taxa_figure


# Create stacked bar figures ----------------------------------------------

ltm <- ggplot(data = target_90, mapping = aes(x = survey_year, y = pct_cover, fill = scientific_name)) + 
  geom_col(position="fill", width=1) + 
  clean + 
  scale_fill_manual(values=natparks.pals("Acadia", 9))+
  labs(x = "Year", y = "Percent Cover") +
  labs(title = "Long-Term Monitoring Program") +
  scale_y_continuous(labels = scales::percent) +
  xlim(1990,2021)

cbs <- ggplot((data = ucsc), mapping = aes(x = year, y = percent_cover, fill = name_1990)) + 
  geom_col(position = "fill", width=1) + 
  clean + 
  scale_fill_manual(values=natparks.pals("Acadia", 9)) +
  labs(x = "Year", y = "Percent Cover") +
  labs(title = "Coastal Biodiversity Survey") +
  scale_y_continuous(labels = scales::percent) +
  xlim(1990,2021)
...
ggplot(comb_limited) +
  geom_point(mapping=aes(NMDS1, NMDS2, color=program, shape=site)) +
  geom_text(data=species.scores,aes(x=NMDS1,y=NMDS2,label=species),size=3,alpha=0.5) + 
  clean+stat_ellipse(mapping=aes(NMDS1, NMDS2, color=program),type = "norm") + 
  scale_color_manual(values=c("#212E52","#D8511D"))

#PERMANOVA (community ~ year * site * program)
